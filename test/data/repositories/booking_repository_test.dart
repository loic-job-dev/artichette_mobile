...
class FakeBookingCreateRequest extends Fake implements BookingCreateRequest {}
...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(FakeBookingCreateRequest());
  });
  ...
  test('creates booking and sends correct request', () async {
    when(() => api.createBooking(any())).thenAnswer(
      (_) async => {
        "id": "1",
    ...
    expect(captured.startBookedDate, DateTime(2026, 1, 10));

    expect(captured.endBookedDate, DateTime(2026, 1, 12));

    expect(captured.adultNumber, 2);

    expect(captured.childrenNumber, 1);

    expect(captured.roomType, 'STD');
  ...
  test('propagates api exceptions', () async {
    when(() => api.createBooking(any())).thenThrow(Exception('API failed'));
